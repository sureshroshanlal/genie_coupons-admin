...
export default function CouponScraper() {
  ...
  const runSingle = async () => {
    if (!slug.trim()) { setSingleError("Slug is required."); return; }
    if (!apiKey.trim()) { setSingleError("Gemini API key is required."); return; }
    setSingleError("");
    setSingleResult(null);
    setSingleStatus("loading");
    try {
      const result = await scrapeStore(slug.trim(), apiKey.trim(), model, backendUrl);
  ...
  const runBatch = async (rowsOverride = null) => {
    ...
    if (!validKeys.length) { setBatchError("Add at least one Gemini API key."); return; }
    const rows = rowsOverride || parseBatch(batchText);
    if (!rows.length) { setBatchError("No slugs found. One slug per line, optional label after comma."); return; }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      setBatchStatus(`[${i + 1}/${rows.length}] ${r.label} — key ${keyEntry.i + 1}`);

      try {
        const result = await scrapeStore(r.slug, keyEntry.k, model, backendUrl);
        setKeyUsage((prev) => ({ ...prev, [`key${keyEntry.i}`]: (prev[`key${keyEntry.i}`] || 0) + 1 }));
        setBatchResults((prev) => [...prev, { slug: r.slug, label: r.label, status: "done", keyUsed: keyEntry.i + 1, ...result }]);
      } catch (err) {
        setBatchResults((prev) => [...prev, { slug: r.slug, label: r.label, status: "error", error: err.message, keyUsed: keyEntry.i + 1 }]);
      }
    }
  ...
  const exportCSV = () => {
    const headers = ["slug", "label", "status", "inserted", "skipped", "total_scraped", "message", "error", "key_used"];
    const rows = batchResults.map((r) =>
      [r.slug, r.label, r.status, r.inserted ?? "", r.skipped ?? "", r.total_scraped ?? "", r.message || "", r.error || "", r.keyUsed || ""]
  ...
  const singleStatusMap = {
    ...
    done: <StatusBadge bg="#EAF3DE" color="#2E5C0E" text={`✓ ${singleResult?.inserted ?? 0} inserted · ${singleResult?.skipped ?? 0} skipped`} />,
    failed: <StatusBadge bg="#FAECE7" color="#993C1D" text={`✗ ${singleResult?.error || "Failed"}`} />,
  };
  ...
  return (
    <div style={{ padding: "1.5rem 0", fontFamily: "var(--font-sans)", maxWidth: 700 }}>
      ...
      <div style={{ background: "#0F2240", color: "#fff", borderRadius: 12, padding: "1.1rem 1.4rem", marginBottom: "1.25rem" }}>
        🔍 Coupon Scraper
        <br />
        <span style={{ fontSize: 12 }}>Scrape store homepages · Parse with Gemini · Save new coupons to DB</span>
      </div>
      ...
      <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "1rem", marginBottom: "1rem" }}>
        ...
        {mode === "single" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div>
              ...
              <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="AIzaSy•••••••••••" style={inputStyle} disabled={singleStatus === "loading"} />
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Model</label>
              <select value={model} onChange={(e) => setModel(e.target.value)} style={{ ...inputStyle, height: 36 }} disabled={singleStatus === "loading"}>
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => setApiKeys((prev) => [...prev, ""])} disabled={batchRunning || apiKeys.length >= 10}
                ...
                {apiKeys.length > 1 && (
                  <button onClick={() => setApiKeys((prev) => prev.slice(0, -1))} disabled={batchRunning}
              ...
              {apiKeys.map((k, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <input type="password" value={k}
                    onChange={(e) => setApiKeys((prev) => prev.map((x, j) => (j === i ? e.target.value : x)))}
                    placeholder="AIzaSy•••••••••••" style={{ ...inputStyle, flex: 1 }} disabled={batchRunning} />
                  {keyUsage[`key${i}`] > 0 && (
            ...
            <div style={{ marginTop: 8 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Model</label>
              <select value={model} onChange={(e) => setModel(e.target.value)} style={{ ...inputStyle, height: 36 }} disabled={batchRunning}>
        ...
        <div>
          ...
          <input value={backendUrl} onChange={(e) => setBackendUrl(e.target.value)} placeholder="https://your-app.onrender.com" style={inputStyle} disabled={batchRunning || singleStatus === "loading"} />
        </div>
      </div>
      ...
      {mode === "single" && (
        <div>
          <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "1rem", marginBottom: "1rem" }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Merchant Slug *</label>
            <input
              value={slug}
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: "1rem" }}>
            <button
              ...
              style={{
                ...
                background: singleStatus === "loading" ? "var(--color-background-tertiary)" : "#0F2240",
                color: singleStatus === "loading" ? "var(--color-text-tertiary)" : "#fff",
                cursor: singleStatus === "loading" ? "not-allowed" : "pointer", fontFamily: "inherit",
              }}
            >
          ...
          {singleError && (
            <div style={{ background: "#FAECE7", border: "0.5px solid #F0997B", borderRadius: 6, padding: "8px 12px", fontSize: 13, color: "#993C1D", marginBottom: "1rem" }}>
              {singleError}
            </div>
          )}

          {singleStatus === "done" && singleResult && (
            <div style={{ background: "#EAF3DE", border: "0.5px solid #97C459", borderRadius: 8, padding: "12px 14px", fontSize: 13, color: "#2E5C0E" }}>
              <div style={{ fontWeight: 500, marginBottom: 6 }}>✓ Scrape complete — {slug}</div>
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap", fontSize: 12 }}>
                <span>🆕 Inserted: <strong>{singleResult.inserted}</strong></span>
                <span>⏭ Skipped (duplicates): <strong>{singleResult.skipped}</strong></span>
                <span>📄 Total scraped: <strong>{singleResult.total_scraped}</strong></span>
              </div>
              {singleResult.message && (
                <div style={{ marginTop: 6, fontSize: 12, opacity: 0.8 }}>{singleResult.message}</div>
              )}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: "0.9rem" }}>
            ...
            <textarea
              ...
              placeholder={"healthyline-coupons, Healthyline\nparsec-coupons, Parsec\nnike-coupons"}
              rows={8}
          ...
          {batchRows.length > 0 && (
            <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, padding: "8px 12px", marginBottom: "0.9rem", fontSize: 13, display: "flex", gap: 20, flexWrap: "wrap" }}>
              <span>📋 <strong>{batchRows.length}</strong> stores</span>
              <span>🔑 <strong>{apiKeys.filter((k) => k.trim()).length}</strong> keys — no delay</span>
            </div>
          )}
          ...
          {batchRunning && (
            <div style={{ marginBottom: "0.9rem" }}>
              ...
              <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginTop: 4 }}>{batchStatus}</div>
            </div>
          )}

          <div style={{ display: "flex", gap: 8, marginBottom: "1rem" }}>
            <button onClick={() => runBatch()} disabled={batchRunning || !batchRows.length}
              style={{ flex: 1, padding: "9px", fontSize: 14, fontWeight: 500, border: "none", borderRadius: 8, background: batchRunning ? "var(--color-background-tertiary)" : "#0F2240", color: batchRunning ? "var(--color-text-tertiary)" : "#fff", cursor: batchRunning ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
              {batchRunning ? `⏳ Scraping ${batchIdx}/${batchTotal}…` : "🚀 Start Bulk Scrape"}
            </button>
            {batchRunning && (
              <button onClick={() => (stopRef.current = true)}
                style={{ padding: "9px 14px", fontSize: 13, border: "0.5px solid #C04828", borderRadius: 8, background: "transparent", color: "#C04828", cursor: "pointer", fontFamily: "inherit" }}>
                ⏹ Stop
              </button>
            )}
          </div>
          ...
          {batchResults.length > 0 && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 500, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <span style={{ color: "var(--color-text-secondary)" }}>
                    🆕 {batchResults.reduce((s, r) => s + (r.inserted || 0), 0)} total inserted
                  </span>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  {batchResults.filter((r) => r.status === "error").length > 0 && !batchRunning && (
                  ...
                  <button onClick={exportCSV}
                    style={{ fontSize: 11, padding: "3px 10px", border: "0.5px solid var(--color-border-secondary)", borderRadius: 4, background: "var(--color-background-primary)", cursor: "pointer", fontFamily: "inherit" }}>
                    ⬇ CSV
                  </button>
                </div>
              </div>

              <div style={{ maxHeight: 360, overflowY: "auto", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8 }}>
                {batchResults.map((r, i) => (
                  <div key={i} style={{
                    padding: "8px 12px", borderBottom: "0.5px solid var(--color-border-tertiary)",
                    display: "flex", alignItems: "center", gap: 10, fontSize: 13,
                    background: r.status === "error" ? "#FFF8F6" : i % 2 === 0 ? "var(--color-background-primary)" : "var(--color-background-secondary)",
                  }}>
                    <span style={{ color: r.status === "done" ? "#2E5C0E" : "#993C1D" }}>
                      {r.status === "done" ? "✓" : "✗"}
                    </span>
                    <span style={{ flex: 1, fontWeight: 500 }}>{r.label}</span>
                    <span style={{ fontSize: 11, color: "var(--color-text-tertiary)" }}>{r.slug}</span>
                    {r.status === "done" && (
                      <>
                        <span style={{ fontSize: 10, padding: "1px 6px", background: "#EAF3DE", color: "#2E5C0E", borderRadius: 3 }}>
                          +{r.inserted} new
                        </span>
                        <span style={{ fontSize: 10, padding: "1px 6px", background: "#f0f0f0", color: "#555", borderRadius: 3 }}>
                          {r.skipped} skip
                        </span>
                      </>
                    )}
                    {r.status === "error" && (
                      <span style={{ fontSize: 11, color: "#993C1D", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {r.error}
                      </span>
                    )}
                    <span style={{ fontSize: 10, padding: "1px 6px", background: "#f0f0f0", color: "#555", borderRadius: 3 }}>
              ...
              {!batchRunning && batchStatus && (
                <div style={{ marginTop: "0.9rem", fontSize: 12, color: "var(--color-text-secondary)" }}>{batchStatus}</div>
              )}
            </div>
          )}
        </div>
